...
/* Gets the $GPRMC message from the GPS module
 * Note: this can be a time-intensive (>1 sec) function, as it requires the GPS module to cycle through an entire message
 */
String GNSSComm::getRMCString()
{
    String input_string = "";
    input_string += consumeBuffer(); //Consumes the buffer and gets the first character
    do
    {
        input_string += readFromI2C(128);
    } while (input_string.indexOf(_BUFFER_CHAR) == -1); //Ends loop when the next buffer is reached
    int RMC_start_index = input_string.indexOf("$GPRMC");
    int RMC_end_index = input_string.indexOf("$", RMC_start_index + 1);
    return input_string.substring(RMC_start_index, RMC_end_index);
}
...
String GNSSComm::getMessage(int timeout)
{
    ...
    while ((millis() - startTime) < timeout)
    {
        ...
        { // If bytes are available
            b1 = b2;
            b2 = I2c.receive();
            if ((b1 == _MU_LOWERCASE) && (b2 == _B_LOWERCASE))
            { // Check if it is a proprietary UBX message (0xB5 0x62)
                I2c.end();
                return readUBXMessageFromI2C(timeout);
            }
            else if ((b1 == _DOLLAR_SIGN) && (b2 == _G_UPPERCASE))
            { // Check if it is a GPS NMEA message ($G)
                I2c.end();
                return readNMEAMessageFromI2C(_G_UPPERCASE, timeout);
            }
            else if ((b1 == _DOLLAR_SIGN) && (b2 == _P_UPPERCASE))
            { // Check if it is a properiatry U-blox NMEA message ($P)
                I2c.end();
...
String GNSSComm::readUBXMessageFromI2C(int timeout)
{
    ...
    long startTime = millis();
    byte header[] =
    { 0xB5, 0x62, 0x00, 0x00, 0x00, 0x00 }; // First two characters and four blank spaces for the rest of the header
    int headerLength = 6;
...
String GNSSComm::readNMEAMessageFromI2C(byte messageTypeId, int timeout)
{
    ...
    while ((millis() - startTime) < timeout)
    {
        ...
        if ((b1 == CR) && (b2 == LF))
        { // If the end of the message has been reached; write the endline to the string and break
            catChar = (char) b1;
            returnString += catChar;
            catChar = (char) b2;
            returnString += catChar;
...
bool GNSSComm::configUbloxGNSSFlightMode(byte mode)
{
    ...
    if (mode > maxValidMode)
    { // Mode is invalid
        return 1;
    }
    byte msg[] =
    { 0xB5, 0x62, 0x06, 0x24, 0x24, 0x00, // Message header - NAV5
    ...
    while ((millis() - startTime) <= timeout)
    {
        ...
        { // ACK received; mesage was sent and received
            return 0;
        }
    }
...
{
    byte msg[] =
    { 0xB5, 0x62, 0x06, 0x24, 0x00, 0x00, 0X2A, 0x84 }; // Poll request for CFG-NAV5
    int msgLength = 8;
    sendMessageToGNSS(msg, msgLength); // Send the message to the GNSS
    ...
    while ((millis() - startTime) <= timeout)
    {
        ...
        if (response.indexOf(expectedResponseHeader) > -1)
        { // This is the poll response
            byte buf[1] =
            { 0 };
